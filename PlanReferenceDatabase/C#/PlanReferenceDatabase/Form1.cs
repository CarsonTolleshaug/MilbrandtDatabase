...
namespace PlanReferenceDatabase
{
    ...
    public partial class Form1 : Form
    {
        ...
        Timer t_retf = new Timer(); // Timer to return focus
        ...
        public Form1()
        {
            ...
            //dgList.Select();
            //dgList.CurrentCell = dgList.Rows[0].Cells[0];
        }
        ...
        private void UpdatePreview(string file)
        {
            ...
            if (file == "")
                wbPreview.DocumentText = "<body style=\"background-color:#EEEEEE\" <div style=\"text-align:center; color:#AAAAAA\">No PDF selected.</div></body>";
            else if (!File.Exists(file))
                wbPreview.DocumentText = "<body style=\"background-color:#EEEEEE\" <div style=\"text-align:center; color:#AAAAAA\">PDF not found.</div></body>";
            else if (wbPreview.Url != new Uri(file))
            {
                wbPreview.Navigate(file);
            }
        }
        private void t_tick(object sender, EventArgs e)
        {
            if (wbPreview.Focused)
            {
